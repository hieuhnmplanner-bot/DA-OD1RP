...
def _apply_freeze(df):
    ...
    if SNAPSHOT_PREV.exists():
        ...
        frozen = []
        for k, cur_end in zip(key, df["end_date_N"]):
            pe = prev_map.get(k)
            # Don co trong seed (DA1RP) -> dung DUNG end_date cua DA1RP (khop tuyet doi thang dao han).
            # Don MOI chua co trong seed -> giu end_date tu tinh (hom nay + remain).
            if pe and str(pe) not in ("", "nan", "NaT"):
